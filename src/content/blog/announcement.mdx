...
SteelMC is a “new” Minecraft server written in Rust. Do not bother updating [*Days Since the Last Rust Minecraft Server*](https://dayssincelastrustmcserver.com/). We have already been on there since October 16, 2025.
...
That was nine months ago. Since May 20, 2026, Steel has matched vanilla block for block across 7,500 randomly selected terrain-generation test chunks: 2,500 in each dimension.

Our vanilla reference extractor fixes generation order and normalizes a few order-dependent behaviors so those comparisons are reproducible. I explain those cases below.

The entity-spawning stage is intentionally disabled for now. We do not want generated worlds filling with entities whose behavior is not implemented yet, and silently discarding them would give a misleading impression of completeness.
...
One Full chunk depends on a 23-by-23 area reaching the Structure Starts stage. Later stages need smaller areas: 7 by 7, then 5 by 5, then 3 by 3. At the final stages, only the center chunk remains. Stack those minimum requirements and they form a pyramid.
...
Lighting has long been one of the conspicuously missing pieces in experimental Minecraft servers. It is easy to see why: light crosses chunk boundaries, and two adjacent chunks cannot safely propagate overlapping updates without coordination. Vanilla moves lighting off the main thread but serializes it through a single engine, making lighting a bottleneck during parallel world generation.
...
Steel preserves a synchronous game tick for gameplay logic, but moves packet processing, chunk scheduling, and chunk sending out of it. Dedicated workers handle gameplay packets throughout the time between ticks, rather than accumulating every packet for one large batch at the start of the next tick as vanilla does. At the tick boundary, Steel drains the finite set of packets already admitted and briefly closes that phase. It can then advance chunks and entities against a stable world before reopening packet processing.
...
The rule behind that design is simple: the game tick must never wait for asynchronous work.
...
This preserves the useful part of a synchronous world model: gameplay changes happen at defined safe points. A slow disk read or chunk dependency does not have to become a server-wide pause.
...
I have explored WebAssembly, scripting languages, and native Rust plugins. The current direction is a native Rust API, giving extensions the same power as ordinary server code. A native plugin could still host WebAssembly, JavaScript, or Lua for people who want a sandbox or higher-level language. Native plugins would be trusted code, so the API boundary will need careful design.
