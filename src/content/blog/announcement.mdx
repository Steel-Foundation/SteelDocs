...
Nine months into the project, Steel's terrain generation has matched vanilla block for block since May 20, 2026, across 7,500 randomly selected test chunks: 2,500 in each dimension.

Steel is still pre-alpha, but it has moved beyond promises. You can connect to it, explore persistent generated worlds, interact with blocks and inventories, and begin building on what is already there.

Most of those nine months have gone into foundations that are almost invisible when you join the server. We focused on getting the difficult underlying systems right, and that work is now beginning to show up in the game.

Early progress can look slow, but each foundation makes later work easier. As these systems mature, new gameplay features need less infrastructure of their own, and visible progress should accelerate.
...
One Full chunk depends on a 23-by-23 area reaching the Structure Starts stage. Later stages need smaller areas: 7 by 7, then 5 by 5, then 3 by 3. At the final stages, only the center chunk remains. Stacked together, those minimum requirements form a pyramid.
...
Those headline numbers need context. Steel used an average of 30.0 logical CPU cores during the measured interval, compared with 4.3 for Fabric. The test therefore measures parallel scaling as well as per-core efficiency. After adjusting for CPU use, Steel still generated 2.71 times as many chunks per CPU-second as Fabric.
...
Years of Bukkit and Paper plugins have already shown us many of the use cases an extension API needs to support. That experience is shaping systems Steel has today, including its [permission system](/configuration/permissions/) and registries. But Steel does not have plugins yet, and we will not publish an API until the server beneath it is mature enough to make that promise responsibly.
