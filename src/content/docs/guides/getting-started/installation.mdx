...
## What to select
If you have already a k8, docker swarn or a normal docker system running the docker container is recommended.
Then the target platform is pelican, pterodactyl or a platform uses eggs. Steel has an egg (json/yaml) created for you.
If you want to run steel native the prebuild binaries are the choice for you.
If your target platform is not support currently of any other thing, you need to cross compile yourself!

Overview which architecture are supported by each solution:
